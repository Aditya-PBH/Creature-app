...
var POSTS = [
  { id: '1', username: 'rahul_s', avatar: 'https://i.pravatar.cc/60?img=1', image: 'https://picsum.photos/seed/a1/400/400', caption: 'Bhai ye jagah gajab hai! 🏔️', likes: 1234 },
  { id: '2', username: 'priya_k', avatar: 'https://i.pravatar.cc/60?img=2', image: 'https://picsum.photos/seed/a2/400/400', caption: 'Ghar ka khana best 🍛', likes: 987 },
  { id: '3', username: 'aman_v', avatar: 'https://i.pravatar.cc/60?img=3', image: 'https://picsum.photos/seed/a3/400/400', caption: 'Gym grind 💪', likes: 543 },
];

var REELS = [
  { id: '1', username: 'rahul_s', avatar: 'https://i.pravatar.cc/60?img=1', image: 'https://picsum.photos/seed/b1/400/700', caption: 'Travel vibe 🔥 #india', likes: 4821 },
  { id: '2', username: 'priya_k', avatar: 'https://i.pravatar.cc/60?img=2', image: 'https://picsum.photos/seed/b2/400/700', caption: 'Food lover 😋 #foodie', likes: 9234 },
  { id: '3', username: 'aman_v', avatar: 'https://i.pravatar.cc/60?img=3', image: 'https://picsum.photos/seed/b3/400/700', caption: 'Gym grind 💪 #fitness', likes: 3102 },
];
    ...
    <SafeAreaView style={st.bg}>
      ...
      <ScrollView contentContainerStyle={st.loginWrap}>
        ...
        <View style={st.card}>
          ...
          <TouchableOpacity
            style={st.loginBtn}
            onPress={function() { props.onLogin({ name: 'Aditya', username: 'adi72' }); }}
          >
            <Text style={st.loginBtnTxt}>Login Karo</Text>
          </TouchableOpacity>
          <View style={st.divRow}>
            <View style={st.divLine} />
            <Text style={st.divTxt}>YA</Text>
            <View style={st.divLine} />
          </View>
          <TouchableOpacity
            style={st.waBtn}
        ...
        renderItem={function(ref) {
          ...
          return (
            <View style={st.storyWrap}>
              <View style={item.own ? st.storyRingOwn : st.storyRingActive}>
          ...
          <View key={post.id} style={st.postCard}>
            ...
            <View style={st.postActs}>
              <TouchableOpacity onPress={function() { toggleLike(post.id); }} style={st.actBtn}>
                <Text style={st.actIcon}>{liked[post.id] ? '❤️' : '🤍'}</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={st.actBtn}
                onPress={function() { doShare('Creature pe dekh: ' + post.caption); }}
              >
                <Text style={st.actIcon}>📤</Text>
              </TouchableOpacity>
            </View>
            <View style={{ paddingHorizontal: 14, paddingBottom: 14 }}>
              <Text style={st.postLikes}>{post.likes + (liked[post.id] ? 1 : 0)} likes</Text>
            ...
            <View key={reel.id} style={{ width: W, height: W * 1.77, position: 'relative' }}>
              ...
              <View style={{ position: 'absolute', right: 12, bottom: 120 }}>
                <TouchableOpacity
                  ...
                  onPress={function() { toggleLike(reel.id); }}
                >
                  <Text style={{ fontSize: 28 }}>{liked[reel.id] ? '❤️' : '🤍'}</Text>
                  <Text style={{ color: '#fff', fontSize: 12 }}>{reel.likes + (liked[reel.id] ? 1 : 0)}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={{ alignItems: 'center' }}
                  onPress={function() { doShare('Creature pe Reel dekh: ' + reel.caption); }}
                >
      ...
      <FlatList
        ...
        renderItem={function(ref) {
          ...
          return (
            <TouchableOpacity
              style={st.chatRow}
              onPress={function() { props.onOpen(item); }}
            >
...
function ChatScreen(props) {
  var [msgs, setMsgs] = useState(MSGS);
    ...
    <SafeAreaView style={st.bg}>
      <View style={st.chatHead}>
        <TouchableOpacity onPress={props.onBack} style={{ padding: 8 }}>
          <Text style={{ color: '#ff3b5c', fontSize: 22 }}>Back</Text>
        </TouchableOpacity>
    ...
    <SafeAreaView style={st.bg}>
      <ScrollView>
        ...
        <View style={st.profBtns}>
          ...
          <TouchableOpacity
            style={st.shareBtn}
            onPress={function() { doShare('Creature pe mera profile dekh! @' + props.user.username); }}
          >
        ...
        <View style={st.grid}>
          {grid.map(function(n) {
            return (
              <Image
                key={n}
                source={{ uri: 'https://picsum.photos/seed/g' + n + '/200/200' }}
                style={st.gridImg}
              />
            );
          })}
...
export default function App() {
  ...
  var [openChat, setOpenChat] = useState(null);
  ...
  if (openChat) {
    return <ChatScreen chat={openChat} onBack={function() { setOpenChat(null); }} />;
  }
    ...
    <SafeAreaView style={st.bg}>
      ...
      {tab !== 'reels' && (
        <View style={st.header}>
          ...
          <TouchableOpacity onPress={function() { setTab('chat'); }}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {tab === 'chat' && <ChatListScreen onOpen={setOpenChat} />}
        {tab === 'profile' && <ProfileScreen user={user} onLogout={function() { setUser(null); }} />}
      </View>
      <View style={st.nav}>
        {TABS.map(function(t) {
          return (
            ...
            >
              <Text style={tab === t.key ? st.navIconActive : st.navIcon}>{t.icon}</Text>
              <Text style={tab === t.key ? st.navLblActive : st.navLbl}>{t.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
var st = StyleSheet.create({
  ...
  storyRingActive: { borderWidth: 2.5, borderColor: '#ff3b5c', borderRadius: 34, padding: 2 },
  storyRingOwn: { borderWidth: 2.5, borderColor: '#555', borderRadius: 34, padding: 2 },
