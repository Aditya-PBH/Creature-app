...
const DUMMY_POSTS = [
  { id: "p1", user: { username: "rahul_s", avatar: "https://i.pravatar.cc/60?img=1" }, image: "https://picsum.photos/seed/post1/400/400", caption: "Bhai ye jagah gajab hai! 🏔️", likes: 1234 },
  { id: "p2", user: { username: "priya_k", avatar: "https://i.pravatar.cc/60?img=2" }, image: "https://picsum.photos/seed/post2/400/400", caption: "Ghar ka khana >> bahar ka 🍛", likes: 987 },
  { id: "p3", user: { username: "aman_v", avatar: "https://i.pravatar.cc/60?img=3" }, image: "https://picsum.photos/seed/post3/400/400", caption: "Gym grind 💪💪", likes: 543 },
];

const DUMMY_REELS = [
  { id: "r1", user: { username: "rahul_s", avatar: "https://i.pravatar.cc/60?img=1" }, image: "https://picsum.photos/seed/reel1/400/700", caption: "Travel vibe 🔥 #india", likes: 4821 },
  { id: "r2", user: { username: "priya_k", avatar: "https://i.pravatar.cc/60?img=2" }, image: "https://picsum.photos/seed/reel2/400/700", caption: "Food lover 😋 #foodie", likes: 9234 },
  { id: "r3", user: { username: "aman_v", avatar: "https://i.pravatar.cc/60?img=3" }, image: "https://picsum.photos/seed/reel3/400/700", caption: "Gym grind 💪 #fitness", likes: 3102 },
];
...
const DUMMY_MESSAGES = [
  ...
  { id: "m2", from: "me", text: "Sab badiya yaar!", time: "10:31" },
    ...
    <SafeAreaView style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.loginScroll}>
        ...
        <View style={s.loginCard}>
          ...
          <TouchableOpacity style={s.loginBtn} onPress={() => onLogin({ name: "Aditya", username: "adi72" })}>
            <Text style={s.loginBtnText}>Login Karo</Text>
          </TouchableOpacity>
          <View style={s.dividerRow}>
            <View style={s.dividerLine} /><Text style={s.dividerText}>YA</Text><View style={s.dividerLine} />
          </View>
          <TouchableOpacity style={s.waBtn} onPress={() => onLogin({ name: "Aditya", username: "adi72" })}>
            <Text style={s.waBtnText}>🟢  WhatsApp se login karo</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
      ...
      {DUMMY_POSTS.map(post => (
        <View key={post.id} style={s.postCard}>
          ...
          <View style={s.postActions}>
            <TouchableOpacity style={s.actionBtn} onPress={() => setLiked(p => ({ ...p, [post.id]: !p[post.id] }))}>
              <Text style={[s.actionIcon, liked[post.id] && { color: "#ff3b5c" }]}>{liked[post.id] ? "❤️" : "🤍"}</Text>
            </TouchableOpacity>
    ...
    <View style={{ flex: 1, backgroundColor: "#000" }}>
      <ScrollView pagingEnabled showsVerticalScrollIndicator={false}>
        {DUMMY_REELS.map(reel => (
          <View key={reel.id} style={[s.reelContainer, { height: width * 1.77 }]}>
            <Image source={{ uri: reel.image }} style={s.reelThumb} resizeMode="cover" />
            <View style={s.reelOverlay}>
              <View style={s.reelActions}>
                <TouchableOpacity style={s.reelActionBtn} onPress={() => setLiked(p => ({ ...p, [reel.id]: !p[reel.id] }))}>
                  <Text style={s.reelActionIcon}>{liked[reel.id] ? "❤️" : "🤍"}</Text>
                  <Text style={s.reelActionCount}>{reel.likes + (liked[reel.id] ? 1 : 0)}</Text>
                </TouchableOpacity>
...
function ChatListScreen({ onOpen }) {
  return (
    <SafeAreaView style={s.container}>
      ...
      <FlatList
        data={DUMMY_CHATS} keyExtractor={i => i.id}
...
function ChatScreen({ chat, onBack }) {
  ...
  const send = () => {
    if (!text.trim()) return;
    setMsgs(p => [...p, { id: Date.now().toString(), from: "me", text: text.trim(), time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
    setText("");
  };
...
const s = StyleSheet.create({
  ...
  storyAddBtn: { position: "absolute", bottom: 0, right: 0, backgroundColor: "#ff3b5c", width: 20, height: 20, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  ...
  reelAvatar: { width: 36, height: 36, borderRadius: 18, marginRight: 8 },
  ...
  msgThem: { backgroundColor: "#1e1e1e", alignSelf: "flex-start", borderBottomLef
